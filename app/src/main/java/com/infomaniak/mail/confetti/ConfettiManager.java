...
public class ConfettiManager {
    ...
    public ConfettiManager setVelocityX(float velocityX) {
        return setVelocityX(velocityX, 0.0f);
    }
    ...
    public ConfettiManager setVelocityY(float velocityY) {
        return setVelocityY(velocityY, 0.0f);
    }
    ...
    public ConfettiManager setAccelerationX(float accelerationX) {
        return setAccelerationX(accelerationX, 0.0f);
    }
    ...
    public ConfettiManager setAccelerationY(float accelerationY) {
        return setAccelerationY(accelerationY, 0.0f);
    }
    ...
    public ConfettiManager setTargetVelocityX(float targetVelocityX) {
        return setTargetVelocityX(targetVelocityX, 0.0f);
    }
    ...
    public ConfettiManager setTargetVelocityY(float targetVelocityY) {
        return setTargetVelocityY(targetVelocityY, 0.0f);
    }
    ...
    public ConfettiManager setRotationalVelocity(float rotationalVelocity) {
        return setRotationalVelocity(rotationalVelocity, 0.0f);
    }
    ...
    public ConfettiManager setRotationalAcceleration(float rotationalAcceleration) {
        return setRotationalAcceleration(rotationalAcceleration, 0.0f);
    }
    ...
    public ConfettiManager setTargetRotationalVelocity(float targetRotationalVelocity) {
        return setTargetRotationalVelocity(targetRotationalVelocity, 0.0f);
    }
    ...
    public void terminate() {
        ...
        if (animationListener != null) animationListener.onAnimationEnd(this);
    }
    ...
    private void addNewConfetti(int numConfetti, long initialDelay, boolean useGaussian) {
        for (int i = 0; i < numConfetti; i++) {
            ...
            confetto.reset();
            configureConfetto(confetto, confettiSource, random, initialDelay, useGaussian);
            confetto.prepare(bound);
            addConfetto(confetto);
        }
    }

    private void startNewAnimation(boolean useGaussian) {
        // Never-ending animator, we will cancel once the termination condition is reached.
        animator = ValueAnimator.ofInt(0).setDuration(Long.MAX_VALUE);
    ...
    private void processNewEmission(long elapsedTime, boolean useGaussian) {
    ...
    private void addConfetto(Confetto confetto) {
        ...
        if (animationListener != null) animationListener.onConfettoEnter(confetto);
    }

    private void removeConfetto(Confetto confetto) {
        if (this.animationListener != null) this.animationListener.onConfettoExit(confetto);
        recycledConfetti.add(confetto);
    }

    protected void configureConfetto(Confetto confetto, ConfettiSource confettiSource, Random random, long initialDelay, boolean useGaussian) {
        ...
        confetto.setInitialVelocityX(getVarianceAmount(velocityX, velocityDeviationX, random, useGaussian));
        confetto.setInitialVelocityY(getVarianceAmount(velocityY, velocityDeviationY, random, useGaussian));
        confetto.setAccelerationX(getVarianceAmount(accelerationX, accelerationDeviationX, random, useGaussian));
        confetto.setAccelerationY(getVarianceAmount(accelerationY, accelerationDeviationY, random, useGaussian));
        confetto.setTargetVelocityX(targetVelocityX == null ? null : getVarianceAmount(targetVelocityX, targetVelocityXDeviation, random, useGaussian));
        confetto.setTargetVelocityY(targetVelocityY == null ? null : getVarianceAmount(targetVelocityY, targetVelocityYDeviation, random, useGaussian));
        confetto.setInitialRotation(getVarianceAmount(initialRotation, initialRotationDeviation, random, useGaussian));
        confetto.setInitialRotationalVelocity(getVarianceAmount(rotationalVelocity, rotationalVelocityDeviation, random, useGaussian));
        confetto.setRotationalAcceleration(getVarianceAmount(rotationalAcceleration, rotationalAccelerationDeviation, random, useGaussian));
        confetto.setTargetRotationalVelocity(targetRotationalVelocity == null ? null : getVarianceAmount(targetRotationalVelocity, targetRotationalVelocityDeviation, random, useGaussian));
        confetto.setTTL(ttl);
        confetto.setFadeOut(fadeOutInterpolator);
    }

    private float getVarianceAmount(float base, float deviation, Random random, boolean useGaussian) {
        // Normalize random to be [-1, 1] rather than [0, 1]
        if (useGaussian) {
            return base + (deviation * ((float) random.nextGaussian() * 2 - 1));
        } else {
