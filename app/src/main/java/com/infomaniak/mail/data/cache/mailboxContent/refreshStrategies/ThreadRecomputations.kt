...
object ThreadRecomputations {
    ...
    private fun Thread.processMessage(message: Message) {
        ...
        from += message.from
        to += message.to
        if (message.isDraft && !message.isMessageWithSendDelay) hasDrafts = true
    ...
    fun Thread.recomputeMessagesWithContent(allMessages: List<Message>) {
        ...
        allMessages.forEach { message ->
            ...
            val isHiddenReminder = message.shouldHideReminder
                    && (isTargetMessageInThread(targetMessageIds, threadMessageIds) || message.isScheduledDraft || isFromSearch)
