...
enum class ThreadListDateDisplay(@DrawableRes val icon: Int?, val formatThreadDate: Context.(Thread) -> String) {
    Default(
        icon = null,
        formatThreadDate = { thread -> defaultFormatting(thread.date) }
    ),
    Scheduled(
        icon = R.drawable.ic_scheduled_messages,
        formatThreadDate = { thread -> relativeFormatting(thread.date) }
    ),
    Snoozed(
        ...
        formatThreadDate = { thread ->
            ...
            val date = if (thread.snoozeState == SnoozeState.Snoozed) thread.snoozeEndDate ?: RealmInstant.MIN else thread.date
